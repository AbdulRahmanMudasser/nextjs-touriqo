...
export default function CalendarPrice() {
  ...
  return (
    <div className="max-w-2xl ml-5 p-4 font-sans -mt-12">
      <h2 className="text-xl font-bold text-gray-800 mb-4">Calendar & Price</h2>
      ...
      <div className="relative">
        <div className="border border-gray-200 rounded-md overflow-hidden">
          ...
          <div className="flex justify-between items-center bg-[#1E2A44] text-white py-1 px-3">
            ...
            <div className="font-medium text-sm">May 2025</div>
            <button className="p-1 hover:bg-gray-700 rounded-full">
              ...
              {week.map((day, dayIndex) => (
                <div
                  ...
                  className={`min-h-[60px] p-1 border-r border-b border-gray-200 last:border-r-0 hover:bg-[#1E2A44] hover:text-white group cursor-pointer relative ${
                    day.isSelected ? "bg-[#ffffff] text-black" : "bg-gray-50 text-gray-800"
                  } ${!day.isCurrentMonth ? "text-black" : ""}`}
                >
                  <div className="text-left">
                    <div className={`text-sm ${day.isSelected ? "text-black" : "group-hover:text-white"}`}>{day.date}</div>
                    {day.price && (
                      <div
                        className={`text-xs mt-1 ${
                          day.isSelected ? "text-[#A3BFFA]" : "text-[#A3BFFA]"
                        } group-hover:text-[#A3BFFA]`} // Keep light blue on hover
                      >
                        {day.price}
                      </div>
                    )}
                  ...
                  <div className="absolute hidden group-hover:block top-[60px] left-0 bg-white shadow-md rounded-md p-3 w-[200px] z-10">
                    <h3 className="text-base font-bold text-gray-800 mb-1">General</h3>
                    <p className="text-gray-700 mb-1 text-sm">General price:</p>
                    <div className="text-xs text-gray-600">
                      <p>
                        Children (0-12): <span className="text-[#A3BFFA]">$109</span> / ticket {/* Keep light blue */}
                      </p>
                      <p>
                        Youth (13-17): <span className="text-[#A3BFFA]">$129</span> / ticket {/* Keep light blue */}
                      </p>
                      <p>
                        Adult (18+): <span className="text-[#A3BFFA]">$169</span> / ticket {/* Keep light blue */}
                      </p>
                    </div>
                  </div>
                </div>
